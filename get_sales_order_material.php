...
$check_duplicate_material="SELECT material FROM material_orders WHERE sales_order_no=$sales_order_no AND material='$material' LIMIT 1";

$check_result=mysqli_query($conn,$check_duplicate_material);

if(mysqli_num_rows($check_result)){
  mysqli_close($conn);
  echo "<script>alert('Material Is Already Added Failed...');window.location:'sales_order.php';</script>";
}
...
$insert2="INSERT INTO material_orders(company,sales_order_no,material,quantity,unit,rate,total_quantity,username) VALUES('$company',$sales_order_no,'$material',$quantity,'$unit',$rate,$quantity,'$username')";
...
<div class="container shadow p-5 mb-4 bg-light">
		...
		<div class="form-row">
	  	...
	  	<label for="selectMaterial">Material</label>
            ...
            <?php
		        ...
		        $query="SELECT material FROM material_details";
  ...
  <?php
    ...
    if(isset($_POST['done'])){
      ...
      echo "<script>alert('Successfully Done...'); window.location='sales_order.php';</script>";
